...
if($txtbuscarpor == 1 && $txtdato != ""){
	...
	where cliente.nombre regexp '$txtdato|$txtdato.'
	order by idhuesped asc");
	
}else if($txtbuscarpor == 2){
	
	$sqlhuesped = $mysqli->query($sql." 
	where cliente.documento  = '$txtdato'");
	
}else if($txtbuscarpor == 3){
  
  $sqlhuesped = $mysqli->query($sql." 
  where cliente.RUC  = '$txtdato'");
  
}else if($txtbuscarpor == 4){
  
  $sqlhuesped = $mysqli->query($sql." 
  where cliente.razon_social  like '%$txtdato%'");
  
} else {
	$sqlhuesped = $mysqli->query($sql." 
	order by cliente.idhuesped asc Limit 0");
}
?>
<!doctype html>
<html>
                    ...
                    <table width="99%" border="0" cellpadding="1" cellspacing="1">
                      <tr>
                        <td width="34%">
                          ...
                          <input name="txtbuscarpor" type="radio" id="rdnombre" value="1">  
                          <label for="rdnombre" class="textoContenidoMenor"> Nombre </label>

                          <input name="txtbuscarpor" type="radio" id="rdruc" value="3">  
                          <label for="rdruc" class="textoContenidoMenor"> RUC </label>
                          <input name="txtbuscarpor" type="radio" id="rdrazon" value="4">  
                          <label for="rdrazon" class="textoContenidoMenor"> Razón social </label>
                        </td>
