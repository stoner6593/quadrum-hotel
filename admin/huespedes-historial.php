...
$sqlhuesped = $mysqli->query("select 
	...
	from cliente inner join estadocivil on estadocivil.idestadocivil = cliente.idestadocivil 
	where cliente.idhuesped = '$xidhuesped' ");
...
$sqlalquiler = $mysqli->query("select
	...
	usuario.user_nombre,
	al_venta.fechafin,
	(select GROUP_CONCAT(tipoperadic_descrip,': Nombre. ',nombre,', DNI. ',dni,', FECHA NAC. ', DATE_FORMAT(nacimiento, '%d/%m/%Y' ) 
					ORDER BY al_venta_personaadicional.id_tipo ASC
					SEPARATOR '<br/>') from al_venta_personaadicional
                    left join al_tipo_peradicional on al_tipo_peradicional.id_tipo = al_venta_personaadicional.id_tipo
                    where al_venta_personaadicional.idalquiler = al_venta.idalquiler) as personales_adicionales	
	from al_venta 
	inner join usuario on usuario.user_id = al_venta.idusuario  
	where idhuesped = '$xidhuesped' order by fecharegistro desc");
                ...
                <tr>
                  <td height="30"><table width="99%" border="0" cellpadding="4" cellspacing="1" bgcolor="#E0E0E0">
                    <tr class="textoContenidoMenor">
                      ...
                      <td width="90" align="center" bgcolor="#F4F4F4">Fecha Hora de Entrada</td>
                      <td width="90" align="center" bgcolor="#F4F4F4">Fecha Hora de Salida</td>
                      <td width="50" height="25" align="center" bgcolor="#F4F4F4">Habitación</td>
                        <td width="135" height="25" align="center" bgcolor="#F4F4F4">Personas Adicionales</td>
                      <td width="50" align="center" bgcolor="#F4F4F4"># Orden </td>
                      <td width="55" align="center" bgcolor="#F4F4F4">Estado</td>
                    ...
                    <tr class="textoContenidoMenor">
                      ...
                      <td align="center" bgcolor="#FFFFFF"><?php $fecha  = strtotime($xaFila['11']); echo Cfecha($xaFila['11']).' - '.date('H:i',$fecha); ?></td>
                      <td height="25" align="center" bgcolor="#FFFFFF"><?php echo $xaFila['3']; ?></td>
                        <td height="25" align="center" bgcolor="#FFFFFF"><?php echo $xaFila['12']; ?></td>
