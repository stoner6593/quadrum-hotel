<?php
	include("generales/Documentoelectronico.php");
	class Invoice extends Documentoelectronico  {
						...
						$writer->startElement("cac:Price");
							$writer->startElement("cbc:PriceAmount");
					        	$writer->startAttribute("currencyID");
					            	$writer->text($this->DocumentCurrencyCode); //Moneda
					       		$writer->endAttribute();
					            	$writer->text($value['pro_preunitario']); //Valor venta
					   		$writer->endElement();
						$writer->endElement();
					$writer->endElement();
				$i++;
							...
							$writer->startElement("cbc:CreditedQuantity");
					   		...
					   		$writer->startElement("cbc:LineExtensionAmount");
					        	$writer->startAttribute("currencyID");
					            	$writer->text($this->DocumentCurrencyCode); //Moneda
					       		$writer->endAttribute();
					            	$writer->text($value['pro_subtotal']); //Valor unitario de venta -  sin IGV
					   		$writer->endElement();
		...
		public function _xmlResumenDiario(){
			...
			$writer->startElement('SummaryDocuments');  
				$writer->writeAttribute('xmlns',$this->xmlnsSummaryDocuments);
				$writer->writeAttributeNS('xmlns','cac', null,$this->cac);
				$writer->writeAttributeNS('xmlns','cbc', null, $this->cbc);
				$writer->writeAttributeNS('xmlns','ds', null, $this->ds);
				$writer->writeAttributeNS('xmlns','ext', null,$this->ext);
				...
				//Datos especificos de factura
				$writer->writeElement("cbc:UBLVersionID", $this->UblVersionId); 
				$writer->writeElement("cbc:CustomizationID", "1.1");
				$writer->writeElement("cbc:ID", $this->getId()); 				 				
				$writer->writeElement("cbc:ReferenceDate",$this->getIssueDate());
				$writer->writeElement("cbc:IssueDate", $this->getIssueDate());
				//Fin datos especificos
				...
				$i=1;
				foreach ($this->getResumenDiario() as $key ) {
				# code...
				
					$writer->startElement("sac:SummaryDocumentsLine");
						$writer->writeElement("cbc:LineID",$i);
						$writer->writeElement("cbc:DocumentTypeCode","03");
						$writer->writeElement("cbc:ID",$key['correlativo']);
						$writer->startElement("cac:AccountingCustomerParty");
							$writer->writeElement("cbc:CustomerAssignedAccountID",$key['RUCReceptor']);
							$writer->writeElement("cbc:AdditionalAccountID","1");
						$writer->endElement();
						$writer->startElement("cac:Status");
							$writer->writeElement("cbc:ConditionCode","1");
						$writer->endElement();
						

						$writer->startElement("sac:TotalAmount");
				        	$writer->startAttribute("currencyID");
				            	$writer->text($this->DocumentCurrencyCode); //Tipo moneda peruana
				       		$writer->endAttribute();
				            	$writer->text(number_format(str_replace(",", "", $key['globalTotalVenta']),2) ); //Total  Factura
				   		$writer->endElement();

				   		$writer->startElement("sac:BillingPayment");
							$writer->startElement("cbc:PaidAmount");
					        	$writer->startAttribute("currencyID");
					            	$writer->text($this->DocumentCurrencyCode); //Tipo moneda peruana
					       		$writer->endAttribute();
					            	$writer->text(number_format(str_replace(",", "", $key['globalGrabadas']),2)); //Valor Factura
					   		$writer->endElement();
					   		$writer->writeElement("cbc:InstructionID","01");	
						$writer->endElement();
						...
						$writer->startElement("cac:TaxTotal");
							$writer->startElement("cbc:TaxAmount");
					        	$writer->startAttribute("currencyID");
					            	$writer->text($this->DocumentCurrencyCode); //Tipo moneda peruana
					       		$writer->endAttribute();
					            	$writer->text(number_format(str_replace(",", "", $key['globalIGV']),2)); //Total IGV
					   		$writer->endElement();
					   		$writer->startElement("cac:TaxSubtotal");
					   			$writer->startElement("cbc:TaxAmount");
						        	$writer->startAttribute("currencyID");
