...
require_once "../../init.php";
   ...
   //              precio,	
			...
			// 	from ventadetalle 
			// 	where idventa = '$this->idalquiler' order by idventadetalle asc");
			 $descripcion="";
			 $items=array();
			...
			// 	$pu1=number_format($tmpFila[5] / 1.18,2);								
			// 	$pu= str_replace(",", "", $pu1);								
			// 	//$t=($pu * 1);
			// 	$t=(str_replace(",", "", $tmpFila[5])); 
			...
			// 	$item ["pro_id"]     = $num;
	  //           $item ["nombre"]   =str_replace('<br>', '', $descripcion) ;
	  //           $item ['cantidad']   = $tmpFila[4];
	  ...
	  //           $sqlactualiza = $link->query("UPDATE  ventadetalle SET procesado=1 where idventadetalle = '$tmpFila[0]'");
			// }
		...
		function _xmlResumen(){
			...
			if($newfecha1 && $newfecha2){
			  ...
			  $concatena=' and DATE(venta.fecharegistro) between "'.$newfecha1.'" and "'.$newfecha2.'" ';
			}

			$sqlventaAlquiler =$link->query("select
			venta.idventa,
			venta.idcliente,
			0,
			'',
			...
			from venta inner join cliente on cliente.idhuesped = venta.idcliente
			where venta.enviado = 1 and venta.ticket is null and venta.iddocumento=1 
			and venta.codigo_respuesta = 0 and venta.anulado = 0 and ".$concatena
			);		
			
			
			if(($sqlventaAlquiler->num_rows==0)){
			...
			$i=0;
			while($xaFila = $sqlventaAlquiler->fetch_row()){
		  ...
		  //           $globalGrabadas+=$st;
				...
				from venta left join ventadetalle on ventadetalle.idventa = venta.idventa
				where venta.idventa = '$xaFila[0]'  order by ventadetalle.idventadetalle asc");
