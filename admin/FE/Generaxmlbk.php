...
require_once "../../init.php";
	...
	class Generaxml extends Invoice
	{
		...
		function _xmlResumen(){
			...
			from al_venta inner join cliente on cliente.idhuesped = al_venta.idhuesped
			where al_venta.enviado = 1 and al_venta.estadoalquiler=1 and al_venta.iddocumento=1 
			and al_venta.codigo_respuesta = 0 and al_venta.anulado = 0 and ".$concatena
			);		
		...
		public function generapdfinvoice($Emcabezado=array(),$NumDocumento,$Datos,$productos=array(),$MontoLetras,$nombre_archivo,$firma='',$NHabitacion=''){		
	       ...
	       switch ($Datos['tipo_documento'])
			...
			$pdf->MultiCell(70,8,$arr['Encabezado']['Receptor']['Direccion'],0,'L',FALSE,1,20,54.5); 
			...
			$pdf->SetXY(3,$end_y);
			$pdf->SetFont('Helvetica','B',8);
			$pdf->Cell(30, 5, "Fecha de Emisión:"); //,FALSE,1,2,$pdf->GetY()

			$pdf->SetXY(30,$end_y);
			$pdf->SetFont('Helvetica','',8);			
			$pdf->Cell(30, 5, $Datos['fecharegistro'],0,0,'L'); 
			//$pdf->SetFont('Helvetica','B',8);
			//$pdf->SetXY(2,44);  
			...
			$pdf->SetFont('Helvetica','B',8);
			$pdf->Cell(30, 5, "Nº Habitación:",0,0,'L');
			$pdf->SetFont('Helvetica','',8);
			$pdf->SetXY(30,$end_y);
			$pdf->Cell(30, 5, $NHabitacion,0,0,'L'); 
			...
			//$yy1=$pdf->GetY();
			$pdf->MultiCell(70,8,"SON: ".$MontoLetras,0,'L',FALSE,1,6,$pdf->GetY()); 
			//$pdf->Ln();
			$pdf->SetX(40);
			$pdf->Cell(40,5,"-----------------------------------------------------------------------------------------------",0,0,'R');
			$pdf->Ln();
			...
			$y=$pdf->GetY();
			$pdf->Ln(5);			
			//$pdf->MultiCell(70,8,$Datos['comentario'],0,'J',FALSE,1,6,$y+5); 
